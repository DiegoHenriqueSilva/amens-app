...
const REPORT_CATEGORIES = [
  ...
  { value: "spam", label: "Spam ou contribuição repetida" },
...
export const ReportContributionDialog = ({ open, contributionId, authorName, onClose, onConfirmed }: Props) => {
  const [category, setCategory] = useState("");
  const [description, setDescription] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleSubmit = async () => {
    if (!category || !contributionId) return;
            ...
            <div className="bg-white rounded-[2rem] shadow-2xl border border-red-100 p-6">
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-foreground">Reportar Contribuição</h3>
                    {authorName && <p className="text-[10px] text-muted-foreground">de {authorName}</p>}
                  </div>
                </div>
